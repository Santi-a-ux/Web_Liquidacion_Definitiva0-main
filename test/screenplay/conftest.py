...
# Only define pytest-html hooks if pytest-html is available
if PYTEST_HTML_AVAILABLE:
    def pytest_html_report_title(report):  # type: ignore[override]
        """Set report title - only called if pytest-html is installed."""
    ...
    def pytest_html_results_summary(prefix, summary, postfix):  # type: ignore[override]
        """Inject quick links and embed artifacts content inside the HTML report - only called if pytest-html is installed."""
        artifacts = [
